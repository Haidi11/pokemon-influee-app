...
const PokemonList = () => {
    ...
    const handleDropPokemon = (pokemon, sourceDeckId, destinationDeckId) => {
        if (!pokemon || sourceDeckId === destinationDeckId) return;

        const updateDecks = (sourceDeck, destinationDeck) => {
            setDeck1((prevDeck) =>
                sourceDeckId === 'deck1'
                    ? prevDeck.filter((p) => p.name !== pokemon.name)
                    : prevDeck
            );
            setDeck2((prevDeck) =>
                sourceDeckId === 'deck2'
            ...
            setDeck1((prevDeck) =>
                destinationDeckId === 'deck1'
                    ? [...prevDeck, sourceDeckId === 'pokemonList' ? currentPokemonRef.current : pokemon]
                    : prevDeck
            );
            setDeck2((prevDeck) =>
                destinationDeckId === 'deck2'
                    ? [...prevDeck, sourceDeckId === 'pokemonList' ? currentPokemonRef.current : pokemon]
                    : prevDeck
            );
    ...
    return (
        <div className="flex">
            ...
            <div className="w-screen h-screen flex justify-center items-center mr-133">
                {selectedPokemon && (
                    <SelectedPokemonDisplay selectedPokemon={selectedPokemon} />
                )}
            </div>
            <Decks 
...
const SelectedPokemonDisplay = ({ selectedPokemon }) => (
